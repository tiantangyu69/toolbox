...
import me.flyness.toolbox.encrypt.base64.Base64InputStream;
import me.flyness.toolbox.encrypt.base64.Base64OutputStream;
...
public class EncryptBase64 {
	...
	public static void encode(File source, File target, int wrapAt)
		...
		try {
			...
			EncryptBase64.encode(inputStream, outputStream, wrapAt);
		} finally {
	...
	public static void encode(File source, File target) throws IOException {
		...
		try {
			inputStream = new FileInputStream(source);
			outputStream = new FileOutputStream(target);
			EncryptBase64.encode(inputStream, outputStream);
		} finally {
			if (outputStream != null) {
				try {
					outputStream.close();
